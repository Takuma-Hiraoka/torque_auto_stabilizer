...
TorqueAutoStabilizer::Ports::Ports() :
...
TorqueAutoStabilizer::TorqueAutoStabilizer(RTC::Manager* manager) : RTC::DataFlowComponentBase(manager),
  ports_(),
  debugLevel_(0)
{
...
RTC::ReturnCode_t TorqueAutoStabilizer::onInitialize(){
  ...
  this->ports_.m_TorqueAutoStabilizerServicePort_.registerProvider("service0", "TorqueAutoStabilizerService", this->ports_.m_service0_);
  this->addPort(this->ports_.m_TorqueAutoStabilizerServicePort_);
...
// static function
bool TorqueAutoStabilizer::readInPortData(const double& dt, const GaitParam& gaitParam, const TorqueAutoStabilizer::ControlMode& mode, TorqueAutoStabilizer::Ports& ports, cnoid::BodyPtr refRobotRaw, cnoid::BodyPtr actRobotRaw, std::vector<cnoid::Vector6>& refEEWrenchOrigin, std::vector<cpp_filters::TwoPointInterpolatorSE3>& refEEPoseRaw, std::vector<GaitParam::Collision>& selfCollision, std::vector<std::vector<cnoid::Vector3> >& steppableRegion, std::vector<double>& steppableHeight, double& relLandingHeight, cnoid::Vector3& relLandingNormal){
  ...
  if(ports.m_steppableRegionIn_.isNew()){
    ...
    if (mode.isABCRunning() && // ABC起動中でないと現在支持脚という概念が無い
        ((gaitParam.footstepNodesList[0].isSupportPhase[RLEG] && !gaitParam.footstepNodesList[0].isSupportPhase[LLEG] && (ports.m_steppableRegion_.data.l_r == torque_auto_stabilizer_msgs::RLEG)) ||
         (gaitParam.footstepNodesList[0].isSupportPhase[LLEG] && !gaitParam.footstepNodesList[0].isSupportPhase[RLEG] && (ports.m_steppableRegion_.data.l_r == torque_auto_stabilizer_msgs::LLEG))) //現在支持脚と計算時支持脚が同じ
        ){
  ...
  if(ports.m_landingHeightIn_.isNew()) {
    ...
    if(std::isfinite(ports.m_landingHeight_.data.x) && std::isfinite(ports.m_landingHeight_.data.y) && std::isfinite(ports.m_landingHeight_.data.z) && std::isfinite(ports.m_landingHeight_.data.nx) && std::isfinite(ports.m_landingHeight_.data.ny) && std::isfinite(ports.m_landingHeight_.data.nz)){
      ...
      if(normal.norm() > 1.0 - 1e-2 && normal.norm() < 1.0 + 1e-2){ // ノルムがほぼ1
        if(mode.isABCRunning()){ // ABC起動中でないと現在支持脚という概念が無い
          if(ports.m_landingHeight_.data.l_r == torque_auto_stabilizer_msgs::RLEG && gaitParam.footstepNodesList[0].isSupportPhase[RLEG] && !gaitParam.footstepNodesList[0].isSupportPhase[LLEG]) { //現在支持脚と計算時支持脚が同じ
          ...
          }else if(ports.m_landingHeight_.data.l_r == torque_auto_stabilizer_msgs::LLEG && gaitParam.footstepNodesList[0].isSupportPhase[LLEG] && !gaitParam.footstepNodesList[0].isSupportPhase[RLEG]) { //現在支持脚と計算時支持脚が同じ
...
bool TorqueAutoStabilizer::execAutoStabilizer(const TorqueAutoStabilizer::ControlMode& mode, GaitParam& gaitParam, double dt, const FootStepGenerator& footStepGenerator, const LegCoordsGenerator& legCoordsGenerator, const RefToGenFrameConverter& refToGenFrameConverter, const ActToGenFrameConverter& actToGenFrameConverter, const Stabilizer& stabilizer, const ExternalForceHandler& externalForceHandler, const FullbodyIKSolver& fullbodyIKSolver,const LegManualController& legManualController, const CmdVelGenerator& cmdVelGenerator) {
  ...
  actToGenFrameConverter.convertFrame(gaitParam, dt,
                                      gaitParam.actRobot, gaitParam.actEEPose, gaitParam.actEEWrench, gaitParam.actCogVel, gaitParam.actRootVel, gaitParam.prevOriginLeg, gaitParam.actZmp);
  ...
  footStepGenerator.procFootStepNodesList(gaitParam, dt, mode.isSTRunning(),
                                          gaitParam.footstepNodesList, gaitParam.srcCoords, gaitParam.dstCoordsOrg, gaitParam.remainTimeOrg, gaitParam.swingState, gaitParam.elapsedTime, gaitParam.prevSupportPhase, gaitParam.relLandingHeight, gaitParam.doubleSupportZmpOffset);
...
// static function
bool TorqueAutoStabilizer::writeOutPortData(TorqueAutoStabilizer::Ports& ports, const TorqueAutoStabilizer::ControlMode& mode, cpp_filters::TwoPointInterpolator<double>& idleToAbcTransitionInterpolator, double dt, const GaitParam& gaitParam){
  if(mode.isSyncToABC()){
...

  {
    ...
    for(int i=0;i<gaitParam.genRobot->numJoints();i++){
      if(mode.now() == TorqueAutoStabilizer::ControlMode::MODE_IDLE || !gaitParam.jointControllable[i]){
...

  {
    ...
    if(mode.now() == TorqueAutoStabilizer::ControlMode::MODE_IDLE){
  ...
  if(mode.isABCRunning() && // ABC起動中でないと支持脚という概念が無い
    ...
    ports.m_landingTarget_.data.l_r = gaitParam.footstepNodesList[0].isSupportPhase[RLEG] ? torque_auto_stabilizer_msgs::RLEG : torque_auto_stabilizer_msgs::LLEG;
...
RTC::ReturnCode_t TorqueAutoStabilizer::onFinalize(){ return RTC::RTC_OK; }

bool TorqueAutoStabilizer::goPos(const double& x, const double& y, const double& th){
  std::lock_guard<std::mutex> guard(this->mutex_);
  if(this->mode_.isABCRunning()){
...
bool TorqueAutoStabilizer::goVelocity(const double& vx, const double& vy, const double& vth){
  std::lock_guard<std::mutex> guard(this->mutex_);
  if(this->mode_.isABCRunning()){
...
bool TorqueAutoStabilizer::goStop(){
  std::lock_guard<std::mutex> guard(this->mutex_);
...
bool TorqueAutoStabilizer::jumpTo(const double& x, const double& y, const double& z, const double& ts, const double& tf){
  std::lock_guard<std::mutex> guard(this->mutex_);
  return true;
}

bool TorqueAutoStabilizer::setFootSteps(const OpenHRP::TorqueAutoStabilizerService::FootstepSequence& fs){
  OpenHRP::TorqueAutoStabilizerService::StepParamSequence sps;
...
bool TorqueAutoStabilizer::setFootStepsWithParam(const OpenHRP::TorqueAutoStabilizerService::FootstepSequence& fs, const OpenHRP::TorqueAutoStabilizerService::StepParamSequence& sps){
  std::lock_guard<std::mutex> guard(this->mutex_);
  if(this->mode_.isABCRunning()){
...
void TorqueAutoStabilizer::waitFootSteps(){
  while (this->mode_.isABCRunning() && !this->gaitParam_.isStatic()) usleep(1000);
  usleep(1000);
  return;
}

bool TorqueAutoStabilizer::releaseEmergencyStop(){
  std::lock_guard<std::mutex> guard(this->mutex_);
  return true;
}


bool TorqueAutoStabilizer::startAutoBalancer(){
...
bool TorqueAutoStabilizer::stopAutoBalancer(){
...
bool TorqueAutoStabilizer::startStabilizer(void){
...
bool TorqueAutoStabilizer::stopStabilizer(void){
...
bool TorqueAutoStabilizer::stopWholeBodyMasterSlave(void){
  std::lock_guard<std::mutex> guard(this->mutex_);
  if(this->mode_.isABCRunning()){
...
bool TorqueAutoStabilizer::setTorqueAutoStabilizerParam(const OpenHRP::TorqueAutoStabilizerService::TorqueAutoStabilizerParam& i_param){
  std::lock_guard<std::mutex> guard(this->mutex_);
...
bool TorqueAutoStabilizer::getFootStepState(OpenHRP::TorqueAutoStabilizerService::FootStepState& i_param) {
  std::lock_guard<std::mutex> guard(this->mutex_);
  ...
  for(int i=0;i<NUM_LEGS;i++){
    ...
    TorqueAutoStabilizer::copyEigenCoords2FootStep(this->gaitParam_.genCoords[i].value(), i_param.leg_coords[i]);
    ...
    TorqueAutoStabilizer::copyEigenCoords2FootStep(this->gaitParam_.srcCoords[i], i_param.leg_src_coords[i]);
    i_param.leg_dst_coords[i].leg = this->gaitParam_.eeName[i].c_str();
    TorqueAutoStabilizer::copyEigenCoords2FootStep(this->gaitParam_.footstepNodesList[0].dstCoords[i], i_param.leg_dst_coords[i]);
  }
  ...
  TorqueAutoStabilizer::copyEigenCoords2FootStep(mathutil::orientCoordToAxis(mathutil::calcMidCoords(this->gaitParam_.footstepNodesList[0].dstCoords, weights), cnoid::Vector3::UnitZ()), i_param.dst_foot_midcoords);
  i_param.is_manual_control_mode.length(NUM_LEGS);
  for(int i=0;i<NUM_LEGS; i++) {
    i_param.is_manual_control_mode[i] = (this->gaitParam_.isManualControlMode[i].getGoal() == 1.0);
  }
...
// static function
void TorqueAutoStabilizer::copyEigenCoords2FootStep(const cnoid::Position& in_fs, OpenHRP::TorqueAutoStabilizerService::Footstep& out_fs){
  out_fs.pos.length(3);
