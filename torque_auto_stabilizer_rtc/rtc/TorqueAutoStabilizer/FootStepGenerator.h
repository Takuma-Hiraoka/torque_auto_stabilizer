...
public:
  ...
  std::vector<std::vector<Eigen::Vector3d> > defaultStrideLimitationHull = std::vector<std::vector<Eigen::Vector3d> >{std::vector<Eigen::Vector3d>{Eigen::Vector3d(0.15,-0.18,0),Eigen::Vector3d(-0.15,-0.18,0),Eigen::Vector3d(-0.15,-0.35,0),Eigen::Vector3d(0.15,-0.35,0)},std::vector<Eigen::Vector3d>{Eigen::Vector3d(0.15,0.35,0),Eigen::Vector3d(-0.15,0.35,0),Eigen::Vector3d(-0.15,0.18,0),Eigen::Vector3d(0.15,0.18,0)}}; // 要素数2. 0: rleg用, 1: lleg用. 単位[m]. goPosやgoVelocityのときに自動生成されるfootstepの、遊脚のエンドエフェクタの着地位置の範囲の凸包. 反対の脚のEndEffector frame(Z軸は鉛直)で表現した着地可能領域(自己干渉やIKの考慮が含まれる). あったほうが扱いやすいのでZ成分があるが、Z成分は0でないといけない. 凸形状で,上から見て半時計回り. thetaの影響はlegHullとlegCollisionMarginを用いて別で評価されるので、defaultStrideLimitationHullでは考慮しなくて良い. 左右方向にsteppable regionをまたぐ場合は、これのY成分が大きくないと後ろ足がまたげない
  ...
  std::vector<std::vector<Eigen::Vector3d> > safeLegHull = std::vector<std::vector<Eigen::Vector3d> >(2, std::vector<Eigen::Vector3d>{Eigen::Vector3d(0.075,0.055,0.0),Eigen::Vector3d(-0.075,0.055,0.0),Eigen::Vector3d(-0.075,-0.055,0.0),Eigen::Vector3d(0.075,-0.055,0.0)}); // 要素数2. rleg: 0. lleg: 1. leg frame. 単位[m]. 凸形状で,上から見て半時計回り. Z成分はあったほうが計算上扱いやすいからありにしているが、0でなければならない. 大きさはgaitParam.legHull以下
  ...
  std::vector<std::vector<Eigen::Vector3d> > overwritableStrideLimitationHull = std::vector<std::vector<Eigen::Vector3d> >{std::vector<Eigen::Vector3d>{Eigen::Vector3d(0.35,-0.15,0),Eigen::Vector3d(-0.35,-0.15,0),Eigen::Vector3d(-0.35,-0.350,0),Eigen::Vector3d(-0.20,-0.45,0),Eigen::Vector3d(0.20,-0.45,0),Eigen::Vector3d(0.35,-0.350,0)},std::vector<Eigen::Vector3d>{Eigen::Vector3d(0.35,0.350,0),Eigen::Vector3d(0.20,0.45,0),Eigen::Vector3d(-0.20,0.45,0),Eigen::Vector3d(-0.35,0.350,0),Eigen::Vector3d(-0.35,0.15,0),Eigen::Vector3d(0.35,0.15,0)}}; // 要素数2. 0: rleg用, 1: lleg用. 着地位置修正時に自動生成されるfootstepの上下限の凸包. 反対の脚のEndEffector frame(Z軸は鉛直)で表現した着地可能領域(自己干渉やIKの考慮が含まれる). あったほうが扱いやすいのでZ成分があるが、Z成分は0でないといけない. 凸形状で,上から見て半時計回り. thetaの影響はlegHullとlegCollisionMarginを用いて別で評価されるので、defaultStrideLimitationHullでは考慮しなくて良い. 斜め方向の角を削るなどして、IKが解けるようにせよ. 歩行中は急激に変更されない
...
protected:
  mutable std::vector<mathutil::FirstOrderLowPassFilter<Eigen::Vector6d> > actLegWrenchFilter = std::vector<mathutil::FirstOrderLowPassFilter<Eigen::Vector6d> >(2, mathutil::FirstOrderLowPassFilter<Eigen::Vector6d>(50.0, Eigen::Vector6d::Zero()));  // 要素数2. rleg: 0. lleg: 1. generate frame. endeffector origin. cutoff 50hz. contactDecisionThresholdを用いた接触判定に用いる
public:
  // startAutoBalancer時に呼ばれる
  void reset(){
    isGoVelocityMode = false;
    for(int i=0;i<NUM_LEGS;i++){
      actLegWrenchFilter[i].reset(Eigen::Vector6d::Zero());
    }
  }
...
public:
  ...
  bool initFootStepNodesList(const GaitParam& gaitParam, const pinocchio::Model& model,
  ...
  public:
    ...
    pinocchio::SE3 coords;
    double stepHeight, stepTime; // stepHeightは0以上でなければならない. stepTimeは0より大きくなければならない
    bool swingEnd;
  public:
    StepNode () : l_r(RLEG), stepHeight(), stepTime(), swingEnd(false){};
    StepNode (const int _l_r, const pinocchio::SE3& _coords, const double _stepHeight, const double _stepTime, const bool _swingEnd)
    ...
    friend std::ostream &operator<<(std::ostream &os, const StepNode &sn) {
      ...
      os << "  rot =";
      os << (sn.coords.rotation()).format(Eigen::IOFormat(Eigen::StreamPrecision, 0, ", ", "", " [", "]")) << std::endl;
...
protected:
  ...
  std::vector<Eigen::Vector3d> calcRealStrideLimitationHull(const int& swingLeg, const double& theta, const std::vector<std::vector<Eigen::Vector3d> >& legHull, const std::vector<mathutil::TwoPointInterpolator<Eigen::Vector3d> >& defaultTranslatePos, const std::vector<std::vector<Eigen::Vector3d> >& strideLimitationHull) const;
  // footStepNodesList[idx:] idxより先のstepの位置をgenerate frameで(左から)transformだけ動かす
  void transformFutureSteps(std::vector<GaitParam::FootStepNodes>& footStepNodesList, int index, const pinocchio::SE3& transform/*generate frame*/) const;
  // footStepNodesList[idx:] idxより先のstepの位置をgenerate frameでtransformだけ動かす
  void transformFutureSteps(std::vector<GaitParam::FootStepNodes>& footStepNodesList, int index, const Eigen::Vector3d& transform/*generate frame*/) const;
  // indexのsupportLegが次にswingするまでの間の位置を、generate frameで(左から)transformだけ動かす
  void transformCurrentSupportSteps(int leg, std::vector<GaitParam::FootStepNodes>& footStepNodesList, int index, const pinocchio::SE3& transform/*generate frame*/) const;
  // footStepNodesの次の一歩を作る. RLEGとLLEGどちらをswingすべきかも決める
  void calcDefaultNextStep(std::vector<GaitParam::FootStepNodes>& footStepNodesList, const GaitParam& gaitParam, const Eigen::Vector3d& offset = Eigen::Vector3d::Zero() /*leg frame*/, bool stableStart = true) const;
  // footStepNodesの次の一歩を作る.
  GaitParam::FootStepNodes calcDefaultSwingStep(const int& swingLeg, const GaitParam::FootStepNodes& footStepNodes, const GaitParam& gaitParam, const Eigen::Vector3d& offset = Eigen::Vector3d::Zero(), bool startWithSingleSupport = false) const;
