...
public:
  ...
  std::vector<bool> jointControllable; // 要素数と順序はnumJoints()と同じ．falseの場合、qやtauはrefの値をそのまま出力する(writeOutputPort時にref値で上書き). 指を位置制御にするため．
...
public:
  ...
  mathutil::TwoPointInterpolatorSE3 footMidCoords = mathutil::TwoPointInterpolatorSE3(cnoid::Position::Identity(),Eigen::Vector6d::Zero(),Eigen::Vector6d::Zero(),mathutil::HOFFARBIB); // generate frame. Z軸は鉛直. 支持脚の位置姿勢(Z軸は鉛直)にdefaultTranslatePosを適用したものの間をつなぐ. interpolatorによって連続的に変化する. reference frameとgenerate frameの対応付けに用いられる
  ...
  mathutil::FirstOrderLowPassFilter<Eigen::Vector6d> actRootVel = mathutil::FirstOrderLowPassFilter<Eigen::Vector6d>(3.5, Eigen::Vector6d::Zero()); // generate frame. 現在のroot速度. actCogVelと同程度
  std::vector<cnoid::Position> actEEPose; // 要素数と順序はeeNameと同じ.generate frame
  std::vector<Eigen::Vector6d> actEEWrench; // 要素数と順序はfsenorNameと同じ.generate frame. この値でインピーダンス制御を行うので、はじめからEE数まではEndEffector origin. それ以降はfsensorParentLink origin．ロボットが受けた力
  ...
  // LegCoordsGenerator
  std::vector<mathutil::TwoPointInterpolatorSE3> genCoords = std::vector<mathutil::TwoPointInterpolatorSE3>(NUM_LEGS, mathutil::TwoPointInterpolatorSE3(cnoid::Position::Identity(),Eigen::Vector6d::Zero(),Eigen::Vector6d::Zero(),mathutil::HOFFARBIB)); // 要素数2. rleg: 0. lleg: 1. generate frame. 現在の目標位置
  ...
  std::vector<cnoid::Position> eeTargetPose; // 要素数と順序はeeNameと同じ.generate frame. astで計算された目標位置姿勢

  // Stabillizer
  cnoid::BodyPtr genRobotTqc;
...
public:
  ...
  void init(const cnoid::BodyPtr& robot){
    ...
    genRobotTqc = robot->clone();
    genRobotTqc->calcForwardKinematics(); actRobot->calcCenterOfMass();
  }
